...
Node.prototype.loadContext = function() {
    var self = this;
    return waterlineService.nodes.findOne({mac: self.mac})
    .then(function(node) {
        self.context = _.defaultsDeep(self.context, node.context);
    });
};

Node.prototype.saveContext = function() {
    var self = this;
    return waterlineService.nodes.update({mac: self.mac}, {context: self.context});
};
    ...
    .then(function(data) {
        ...
        if (extractor) {
            ...
            if (matches) {
                ...
                self.context[extractor.referenceName] = matches[matchNumber];
            }
        }
