...
NodeService.prototype.postTaskByContextNodeId = function(nodeId, taskName) {
    var taskId = uuid.v4();
    return Promise.all([
        waterlineService.nodes.findOne({'context.nodeId' : nodeId}),
        taskManager.getJobsByName(taskName)
    ])
    .spread(function(node, jobs) {
        if(_.isEmpty(node)) {
